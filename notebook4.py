...
def eliminar_doctor():
    ...
    if seleccionado1:
        indice1=int(seleccionado1[0])
        id_item1=seleccionado1[0]
        if messagebox.askyesno("Eliminar Doctor/a", f"¿Está seguro de eliminar al doctor/a '{TreevieW.item(id_item1, 'values')[0]}'?"):
            del doctores_data[indice1]
            guardar_en_archivo_doctores() # GUARDAR LOS CAMBIOS EN EL ARCHIVO
            Cargar_treeview()
            messagebox.showinfo("Eliminar Doctor/a", "Doctor/a eliminado exitosamente.")
        else: #este else es del if seleccionado
            messagebox.showwarning("Eliminar Doctor/a", "No se ha seleccionado ningun doctor/a.")
#NOMBRE
...
#BOTON REGISTRAR
boton_registrar=tk.Button(boton_frame, text="Registrar", command=registrar_doctor)
boton_registrar.grid(row=5, column=0, padx=5)
boton_registrar.configure(background="green")
#BOTON ELIMINAR
boton_eliminar=tk.Button(boton_frame, text="Eliminar", command=eliminar_doctor)
...
#DEFINIR ANCHOS DE COLUMNAS
TreevieW.column("Nombre", width=120)
TreevieW.column("Especialidad", width=180)
TreevieW.column("Edad", width=50, anchor="center")
